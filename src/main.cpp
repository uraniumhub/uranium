...
#if defined(NDEBUG)
# error "Uranium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1a85cdfe0d106b183e2a0a1f01a6658d63c7ade411e838e91d7960a42a72d65e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uranium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Uranium Signed Message:\n";
...
{
    // Uranium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 262800 blocks, which will occur approximately every 6 months
    nSubsidy >>= (nHeight / 262800); // Uranium: 262800 blocks in ~6 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Uranium: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Uranium: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Uranium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Uranium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a85cdfe0d106b183e2a0a1f01a6658d63c7ade411e838e91d7960a42a72d65e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 3/Nov/2017 US Bombs ISIS in Somalia for the First Time";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047729410fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1509698979;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1697129;
        ...
        assert(block.hashMerkleRoot == uint256("0x2373673b7f1e6b8151f8c25d002fc70497fb02007793e2138f6ec504ecdfa75f"));
...
unsigned char pchMessageStart[4] = { 0xcd, 0xd4, 0xc7, 0xad }; // Uranium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UraniumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UraniumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UraniumMiner(CWallet *pwallet)
{
    printf("UraniumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uranium-miner");
    ...
    try { loop {
        ...
        printf("Running UraniumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UraniumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UraniumMiner, pwallet));
}
